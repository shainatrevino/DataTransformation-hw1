...
```{r keys}
student <- student %>% count(id_student, code_presentation, code_module) %>% filter(n>1)

assessment <- assessment %>% count(id_student, id_assessment) %>%  filter(n>1)

as_d <- as_d %>% count(code_module, id_assessment) %>%  filter(n>1)
